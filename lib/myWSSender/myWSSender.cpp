...
void myWSResponder::prepareWSData(const int8_t _i8messageType, AsyncWebSocketClient * _client) {
  Serial.printf("- myWSResponder::prepareWSData. starting. Message type [%i]\n", _i8messageType);
  StaticJsonDocument<900> __doc;
  JsonObject __newObj = __doc.to<JsonObject>();
  __newObj["action"] = _i8messageType;
  ...
  if (_i8messageType == 0 /*|| _i8messageType == 1*/ || _i8messageType == 2) {
    ...
    __newObj["message"] = _messages_array[_i8messageType];
    if (globalBaseVariables.MY_DG_WS) {
      Serial.printf("- myWSResponder::prepareWSData. _messages_array[%i] = %s\n", _i8messageType, _messages_array[_i8messageType]);
    }
    // expected JSON obj: {"action":0;"message":"Hello WS Client"}
    // expected JSON obj: {"action":1;"message":"I got your WS text message"}
    // expected JSON obj: {"action":2;"message":"I got your WS binary message"}
  ...
  if (_i8messageType == 3) {
     ...
     * "mesh":{"mssid":"laser_boxes","mpass":"somethingSneaky","mport":5555}} */
    if (globalBaseVariables.MY_DG_WS) {
      Serial.printf("- myWSResponder::prepareWSData. Message type [%i]. About to allot __newObj[\"serverIP\"] = (globalBaseVariables.laserControllerMesh.getStationIP()).toString()\n", _i8messageType);
      Serial.printf("- myWSResponder::prepareWSData. Message type [%i]. server IP ", _i8messageType);Serial.println((globalBaseVariables.laserControllerMesh.getStationIP()).toString());
    }
...
void myWSResponder::sendWSData(JsonObject& _joMsg, AsyncWebSocketClient * _client) {
    if (globalBaseVariables.MY_DG_WS) {
      Serial.println("- myWSResponder::sendWSData. starting");
    }
    ...
    serializeJson(_joMsg, (char *)_buffer->get(), _len + 1);
    // Display the serialized message to the console
    Serial.print("- myWSResponder::sendWSData. Serialized message: ");serializeJson(_joMsg, Serial);Serial.println();
    ...
    if (globalBaseVariables.MY_DG_WS) {
      Serial.println(F("- myWSResponder::sendWSData. over"));
    }
}
...
void myWSSenderTasks::_tcbSendWSDataIfChangeStationIp() {
  ...
  myWSSender _myWSSender(_server, tSendWSDataIfChangeStationIp);
  if (globalBaseVariables.MY_DG_WS) {
